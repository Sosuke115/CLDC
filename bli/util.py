...
def load_lines(path):
    return [
        l.strip()
        for l in list(
            codecs.open(path, "r", encoding="utf8", errors="replace").readlines()
        )
    ]
    ...
    with codecs.open(path, encoding="utf8", errors="replace") as f:
        ...
        while line:
            ...
            if len(line[1:]) == dimension:
                ...
                print(
                    "Error in the embeddings file, line "
                    + str(cntr)
                    + ": unexpected vector length (expected "
                    + str(dimension)
                    + " got "
                    + str(len(np.array(line[1:])))
                    + " for word '"
                    + line[0]
                    + "'"
                )
...
def write_embs(path, vocab, embs):
    if (len(vocab)) != embs.shape[0]:
        raise ValueError(
            "Something wrong: vocabulary and emb. matrix don't have the same number of entries"
        )
    ...
    for i in range(len(vocab)):
        ...
        f.write(
            inv_dict[i]
            + " "
            + " ".join([str(round(float(x), 6)) for x in embs[i]])
            + " \n"
        )
    f.close()
...
def serialize_embs(
    path_vocab, path_embs, vocab_dict, embeddings, emb_norm=True, vocab_inv=True
):
    ...
    if vocab_inv:
        pickle.dump(
            {v: k for k, v in vocab_dict.items()}, open(path_vocab + ".inv", "wb+")
        )
...
def deserialize_embs(path_vocab, path_embs, emb_norm=True, vocab_inv=True):
    ...
    # rbはバイナリ読み込み
    vocab_dict = pickle.load(open(path_vocab, "rb"))
    # vocab_dict = pickle.load(open(path_vocab,"r"))
    if emb_norm:
...
def load_and_serialize_embs(
    path_load, path_vocab_ser, path_embs_ser, topk=None, dimension=None
):
    voc, embs = load_embs(path_load, topk, dimension)
    print(len(voc))
    print(embs.shape)
    serialize_embs(
        path_vocab_ser, path_embs_ser, voc, embs, emb_norm=False, vocab_inv=False
    )
...
def big_matrix_csls(a, b, csls_a, csls_b, chunk_size=100):
    ...
    for i in range(num_iters):
        ...
        csls_mat = (
            np.zeros((size_a, b.shape[1]))
            + np.reshape(csls_a[i * chunk_size : i * chunk_size + size_a], (size_a, 1))
            + np.reshape(csls_b, (1, size_b))
        )
